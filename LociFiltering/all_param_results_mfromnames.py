...
def main(argv):
	...
	#Arrange subdirs so that they are ordered by m value
	subdirs_mvalues = []
	for dir in subdirs:
		m_value_name = dir.split("M")
		m_value_name = m_value_name[0]
		subdirs_mvalues.append((dir,int(m_value_name[1:])))

	subdirs_mvalues = sorted(subdirs_mvalues, key=lambda filename: filename[1])
	
	subdirs_ordered = []
	for subdir in subdirs_mvalues:
		dir = subdir[0]
		subdirs_ordered.append(dir)
	...
	subdir_header = "sample,"+",".join(subdirs_ordered)+"\n"
